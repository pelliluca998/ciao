					...
					{!! Form::model($event, ['method' => 'PATCH','files' => true, 'route' => ['events.update', $event->id]]) !!}
						...
						<div class="form-group">
						    ...
						    <div class="form-group" style="width: 48%; float: left;">
									...
									{!! Form::select('spec_iscrizione', EventSpec::leftJoin('types', 'types.id', '=', 'event_specs.id_type')->where('id_event', $event->id)->where('types.label', 'text')->orderBy('event_specs.id')->pluck('event_specs.label', 'event_specs.id'), null, ['class' => 'form-control']) !!}
						    </div>
						</div>
