					...
					<div style="float: left; width: 100%; padding: 5px;">
							...
							<table class='testgrid draggable'>
								...
								<tbody>
									...
									<tr>
										...
										<td>{{$spec->label}}</td>
										<td><input name="week_filter[{{$w}}][]" value="{{$spec->id}}" type="checkbox" onchange="disable_select(this, 'week_filter_value_{{$w}}_{{$index}}', true)"/></td>
										<td>

											@if($spec->id_type>0)
											{!! Form::select('week_filter_value['.$w.'][]', TypeSelect::where('id_type', $spec->id_type)->orderBy('ordine', 'ASC')->pluck('option', 'id'), '', ['class' => 'form-control', 'disabled' => 'true', 'id' => "week_filter_value_".$w."_".$index])!!}
											@else
											@if($spec->id_type==-1)
											{!! Form::text('week_filter_value['.$w.'][]', '', ['class' => 'form-control', 'disabled' => 'true', 'id' => "week_filter_value_".$w."_".$index]) !!}
											...
											{!! Form::number('week_filter_value['.$w.'][]', '', ['class' => 'form-control', 'disabled' => 'true', 'id' => "week_filter_value_".$w."_".$index]) !!}
											@elseif($spec->id_type==-4)
											{!! Form::select('week_filter_value['.$w.'][]', Group::where('id_oratorio', Session::get('session_oratorio'))->orderBy('nome', 'ASC')->pluck('nome', 'id'), '', ['class' => 'form-control', 'disabled' => 'true', 'id' => "week_filter_value_".$w."_".$index])!!}
											@endif
											@endif
						...
						<table class='testgrid draggable'>
							...
							<tr>
								...
								<td>{{$spec->label}}</td>
								<td>
									<input name="spec_filter[]" value="{{$spec->id}}" type="checkbox" onchange="disable_select(this, 'spec_filter_value_{{$loop->index}}', true)"/></td>
									<td>
										...
										@elseif($spec->id_type==-4)
										{!! Form::select('spec_filter_value[]', Group::where('id_oratorio', Session::get('session_oratorio'))->orderBy('nome', 'ASC')->pluck('nome', 'id'), '', ['class' => 'form-control', 'disabled' => 'true', 'id' => "spec_filter_value_".$loop->index])!!}
										@endif
										@endif
